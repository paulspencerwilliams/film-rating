(ns film-ratings.views.film
  (:require
    [hiccup.form :refer [form-to label text-area text-field submit-button]]
    [film-ratings.views.template :refer [page labeled-radio]]
     ...
     [:div
      (form-to [:post "/add-film"]
               ...
               [:div.form-group.col-12
                (label :name "Name:")
                (text-field {:class       "mb-3 form-control"
                             :placeholder "Enter a film name"}
                            :name)]
               [:div.form-group.col-12
                (label :description "Description:")
                (text-area {:class       "mb-3 form-control"
                            :placeholder "Enter a film description"}
                           :description)]
               [:div.form-group.col-12
                (label :rating "Rating (1-5):")
...
(defn film-attributes-view [name description rating]
  [:div
   [:div.row
    [:div.col-2 "Name:"]
    [:div.col-10 name]]
